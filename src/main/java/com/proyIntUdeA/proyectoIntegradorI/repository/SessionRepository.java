...
public interface SessionRepository extends JpaRepository<SessionEntity, Long> {
    ...
    @Query(value = "SELECT c.class_id, c.class_date, s.subject_name, c.registered, c.canceled_by, c.accepted, c.class_topics, c.class_rate, t.user_id, t.user_firstname, t.user_lastname " +
            "FROM class c " +
    ...
    @Query(value = "SELECT c.class_id, c.class_date, s.subject_name, c.registered, c.canceled_by, c.accepted, c.class_topics, c.class_rate, e.user_id, e.user_firstname, e.user_lastname " +
            "FROM class c " +
            "JOIN subject s ON s.subject_id = c.subject_id " +
    ...
    @Query(value = "SELECT c.class_date from class c where c.tutor_id = :tutorId AND c.canceled_by = 0", nativeQuery = true)
    List<Object[]> findDates(@Param("tutorId") String tutorId);
